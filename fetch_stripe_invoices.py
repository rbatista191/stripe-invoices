...
def fetch_invoices():
    ...
    for invoice in invoices.auto_paging_iter():
        invoice_data = {
            'id': invoice.id,
            'created': datetime.fromtimestamp(invoice.created).strftime('%Y-%m-%d %H:%M:%S'),
            ...
            'tax': (invoice.tax if invoice.tax is not None else 0) / 100,  # Stripe amounts are in cents
            'country': invoice.customer_address['country'] if invoice.customer_address else 'N/A'
        }
